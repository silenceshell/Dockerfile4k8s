...
ENV PERCONA_XTRADB_VERSION 5.7
...
RUN echo "deb http://mirrors.tuna.tsinghua.edu.cn/percona/apt trusty main" > /etc/apt/sources.list.d/percona.list
RUN echo "deb-src http://mirrors.tuna.tsinghua.edu.cn/percona/apt trusty main" >> /etc/apt/sources.list.d/percona.list
...
RUN { \
                echo percona-server-server-${PERCONA_XTRADB_VERSION} percona-server-server/data-dir select ''; \
                echo percona-server-server-${PERCONA_XTRADB_VERSION} percona-server-server/root_password password ''; \
        } | debconf-set-selections \
        && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --force-yes percona-xtradb-cluster-client-"${MYSQL_VERSION}" \
