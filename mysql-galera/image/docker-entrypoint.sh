...
if [ "$1" = 'mysqld' ]; then
  ...
  # fail if user didn't supply a root password  
    ...
    echo 'Running mysql_install_db ...'
        rm -rf ${DATADIR}/*
        mysqld --user=mysql --initialize-insecure --datadir="$DATADIR"
...
#        echo "GRANT ALL ON \`$MYSQL_DATABASE\`.* TO '$MYSQL_USER'@'%' ;" >> "$tempSqlFile"
    ...
    # this is the Single State Transfer user (SST, initial dump or xtrabackup user)
      ...
      echo "GRANT SELECT, PROCESS, RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO '${WSREP_SST_USER}'@'localhost';" >> "$tempSqlFile"
    fi
